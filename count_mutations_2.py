...
class AlignBlock():
	...
	def print_str(self, filevar):
		...
		if same_directions:
			...
			l = len(self.s_seq)
			align_sites_2 = ','.join(map(str,sorted([l - i for i in align_sites_2])))
		align_sites_1 = ','.join(map(str,sorted([i - min(self.q_start, self.q_end) for i in self.q_sites])))
...
build_blast_inp_fasta(opt.fasta_path_1, editing_site_dict_raw_1, opt.prefix)
tblastx_run(opt.fasta_path_2, opt.prefix)


align_coord_dict = blastout_parse(opt.prefix, editing_site_dict_raw_1, editing_site_dict_raw_2)
